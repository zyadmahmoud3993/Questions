...
function startQuiz(index) {
    ...
    quiz_item.forEach(item => {
        ...
        item.addEventListener('click', () => {
            ...
            item.textContent.trim() == quiztion_random[index].answer.trim() ? [item.classList.add('true'), curect++, curect_voice.play()] : [item.classList.add('false'), window.navigator.platform.includes('Win') ? error_voice.play() : navigator.vibrate[300]];
            quiz_item.forEach(item => {
