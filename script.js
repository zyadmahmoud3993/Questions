...
function startQuiz(index) {
    ...
    quiz_item.forEach(item => {
        ...
        item.addEventListener('click', () => {
            ...
            setTimeout(()=>{item.classList.remove('hover')},1250)
            item.textContent.trim() == quiztion_random[index].answer.trim() ? [item.classList.add('true'), curect++, curect_voice.play()] : [item.classList.add('false'), window.navigator.platform.includes('Win') ? error_voice.play() : window.navigator.vibrate([2000])];
            quiz_item.forEach(item => {
